...
<img width="1600" height="818" alt="001" src="https://github.com/user-attachments/assets/a77eec6b-f2c2-4322-88f8-e7260fc37f06" />
<img width="1600" height="818" alt="002" src="https://github.com/user-attachments/assets/74b0d7e9-9aa2-4aba-92a6-d0381cb3e990" />


```text
【Camera & Adapter】                          【PawPrint (Type-C Breakout)】

                 +---------------------+
                 |                     |
[Tip (芯線)] ----+---------------------+------> [ A5 / B5 ピン ] (CC1/CC2)
                 |                     |
                 |      [ 10µF ]       |  <-- RC遅延コンデンサ (Delay Capacitor)
                 |                     |
[Sleeve (GND)] --+---------------------+------> [ GND ピン ]
                 |                     |
                 +---------------------+
...
3.5mmオーディオケーブルの片方を切り落とし、内部の「芯線（プラス）」と「シールド線（マイナス/GND）」を剥き出しにします。
...
芯線と10µFコンデンサの片方の足をねじり合わせ、Type-C基板の A5 (または B5) ピンにハンダ付けします。
...
シールド線とコンデンサのもう片方の足をねじり合わせ、基板の GND ピンにハンダ付けします。
Twist the Sleeve wire with the other leg of the capacitor and solder them to the GND pin.

完成した基板をPawPrintのUSB-Cポートに接続し、3.5mmプラグをカメラのホットシューアダプターに接続します。
...
トリガー追加 / Add Trigger: 「外部電圧検出（External Voltage）」

測定モード / Measurement Mode: 「高電平（High Level / 内蔵プルアップ）」

目標電圧範囲 / Target Voltage Range: 0.80V 〜 1.50V
...
強度の一時的な変更 / Temporary Intensity Change: 「パラメータ決定（参数决定）」を選択し、お好みの強度（例: +10 ~ +40）に設定します。/ Select 'Parameter Determination' and set your desired intensity.
